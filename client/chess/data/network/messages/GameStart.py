...
class GameStartResponse(BaseResponse):
    message_type = 3
    def __init__(self, session_id: str, message_id: str, user_id: int, turn: int) -> None:
        super().__init__(session_id, user_id, message_id)
        self.turn = turn


    @classmethod
    def from_dict(cls, message: Dict[str, Any]) -> Type['GameStartResponse']:
        if message['message_type'] != cls.message_type:
            raise ValueError('incorrect message type')
        
        payload = message['payload']
        return GameStartResponse(payload['session_id'], message['message_id'],
                                 payload['user_id'], payload['turn'])
