...
class VoluntarySurrender(BaseMessage):
    message_type = 5
...
class VoluntarySurrenderResponse(BaseResponse):
    message_type = 5
    ...
    @classmethod
    def from_dict(cls, message: Dict[str, Any]) -> Type['VoluntarySurrenderResponse']:
        if message['message_type'] != cls.message_type:
            raise ValueError('incorrect message type')
        
        payload = message['payload']
        return VoluntarySurrenderResponse(payload['session_id'], message['message_id'],
